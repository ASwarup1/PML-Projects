...
save(modFitDT, file="./ModelDT.RData")
...
Here we'd use the 'gbm' method (Boosting with Trees) of the 'caret' package for prediction. We use trainControl function to specify sampling parameters in the model. The object that is outputted from traincontrol is provided as an argument for train. See, e.g., the tutorial mentioned below for explanations:
<http://www.rebeccabarter.com/blog/2017-11-17-caret_tutorial/>
Again, the Confusion Matrix is generated to validate accuracy of prediction against the independent dataset myTest.
...
save(modFitBoost, file="./ModelBoost.RData")

```   
Print and plot of the Boost model's prediction on the training dataset, as well as prediction on the myTest dataset: 
...
confMatBoost
```   
Plot the Boost model's accuracy versus Boosting Iterations.

```{r Boost3, message=FALSE, warning=FALSE}
# plot(confMatBoost$table, col = confMatBoost$byClass,
#      main = paste("Boosting - Accuracy =", round(confMatBoost$overall['Accuracy'], 4)))
# plot(modFitBoost, ylim=c(0.8, 1))
plot(modFitBoost)
...
We'd run the Random Forest algorithm for prediction by using the 'rf' method in the 'caret' package. The standard randomForest() function of R requires tuning of two parameters, namely:
- **mtry**: Number of variables randomly sampled as candidates at each split, and
- **ntree**: Number of trees to grow
In 'caret' only mtry parameter is available for tuning. See, e.g., methods for tuning at: 
<https://machinelearningmastery.com/tune-machine-learning-algorithms-in-r/>
However, for the sake of simplicity we'd leave it as default. One can get higher accuracy by increasing ntree - upto a limit - then it flattens. We chose ntree = 100.
...
save(modFitRF, file='./ModelRF.RData')
...
End_Time  <- Sys.time()
totalTime <- round(End_Time - Begin_Time, 2)
print(paste("Total Execution Time = ", totalTime, " mins"))
